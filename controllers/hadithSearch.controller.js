  ...
  searchUsingSiteSunnah = catchAsync(async (req, res, next) => {
    ...
    const result = Array.from(doc.querySelectorAll('.boh')).map(
      (info) => {
        ...
        const { numero, hadithNumberInCollection } =
  ...
  getOneBookInCollectionUsingSiteSunnah = catchAsync(
    async (req, res, next) => {
      ...
      elements.forEach((element) => {
        if (
          ...
          if (/^C\d+\.\d+$/.test(chapterId)) {
            ...
            const mainChapterId = isMain
              ? chapterId
              : getMainChapterId(chapterId);
      ...
      for (const [chapterId, chapterData] of chaptersMap) {
        ...
        const mainChapterInfo = extractChapterInfo(
          mainChapter.elements,
        );
        const mainChapterAhadith = extractActualHadithContainer(
          mainChapter.elements,
          collectionId,
          bookId,
        );
        ...
        for (const [id, data] of chaptersMap) {
          if (
            !data.isMainChapter &&
            data.mainChapterId === mainChapterId
          ) {
            const subChapterInfo = extractChapterInfo(data.elements);
            const subChapterAhadith = extractActualHadithContainer(
              data.elements,
              collectionId,
              bookId,
            );
        ...
        result.push({
          ...
          numberOfHadithInAllSubchapters: subchapters.reduce(
            (acc, subchapter) => acc + subchapter.numberOfHadith,
            0,
          ),
